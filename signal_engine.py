...
def calc_vwap_bands(df: pd.DataFrame):
    ...
    last = df.index[-1]
    return round(vwap.iloc[-1], 2), round(dev.iloc[-1], 2)

def calc_delta(df: pd.DataFrame) -> float:
    df = df.copy()
    df["delta"] = np.where(df["close"] >= df["open"], df["volume"], -df["volume"])
    return round(df["delta"].iloc[-6:].sum(), 0)
...
def classify_regime(df: pd.DataFrame) -> tuple:
    ...
    if abs(z) < 0.6:
        return "Mean-reverting", "Fade extremes at key levels. VWAP reversion trades preferred.", "#7F77DD"
    elif sma5 > sma20:
...
def get_signals(df: pd.DataFrame, vol_mult: float) -> list:
    ...
    # 3. Extended above +1σ — fade
    if close > vwap + sig:
        ext = round((close - (vwap + sig)) / atr, 1)
        conf = min(99, int(62 * (2.0 - min(vol_mult, 1.8))))
        signals.append({"dir": "S", "name": "VWAP +1σ fade",
                        "detail": f"Price {ext} ATR above VWAP upper band — overextended",
                        "conf": conf, "src": "VWAP+σ"})

    # 4. Extended below -1σ — mean revert long
    if close < vwap - sig:
        ext = round(((vwap - sig) - close) / atr, 1)
    ...
    bars_up = (df["close"].diff().iloc[-4:] > 0).sum()
    ...
    bars_dn = (df["close"].diff().iloc[-4:] < 0).sum()
...
def get_key_levels(df: pd.DataFrame) -> list:
    ...
    pdh = round(prev_day["high"].max(), 2)
    pdl = round(prev_day["low"].min(), 2)
    ...
    levels = [
        ...
        {"price": round(close,         2), "tag": "P", "label": "Current price"},
